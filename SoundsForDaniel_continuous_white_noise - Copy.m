...
impDurL = round(fs * 0.04); % 40 ms 
impDurH = round(fs * 0.03); % 30 ms 

chunkL = y1(10001:10001 + impDurL); % Pick a random chunk from the middle
...
rate1 = 3; % 3 Hz presentation
rate2 = 5; % 5 Hz presentation

seq1 = r; % Initialize with reduced amplitude white noise
seq2 = r; % Initialize with reduced amplitude white noise

% Insert impulses at specific intervals into the white noise
for ss = 0.001:1 / rate1:1
    startSamp = round(ss * fs);
    seq1(startSamp:startSamp + impDurL) = seq1(startSamp:startSamp + impDurL) + impL;
end
subplot(4, 2, 8);
plot(seq1);
...
for ss = 0.001:1 / rate2:1
    startSamp = round(ss * fs);
    seq2(startSamp:startSamp + impDurH) = seq2(startSamp:startSamp + impDurH) + impH;
end
figure;
plot(seq2);
title('Impulse Train H (8-16 kHz)');

audiowrite('low_with_noise.wav', seq1, fs);
audiowrite('high_with_noise.wav', seq2, fs);
